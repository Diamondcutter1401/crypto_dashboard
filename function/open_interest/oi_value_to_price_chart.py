...
def chart_display():
    ...
    ax2 = ax1.twinx()
    ax2.set_ylabel('Open Interest Value', color='tab:orange')
    ax2.plot(df['DateTime'], df['Open Interest Value'], color='tab:orange', label='Open Interest Value')
    ...
    plt.title(f'Open Interest Value and {coin} Price Over Time')
    fig.tight_layout()  # Adjust layout to make room for the labels
    plt.show()

    response = input("Wanna see other pairs?")
